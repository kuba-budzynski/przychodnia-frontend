...
function profile({user}) {
  ...
  const {data, error} = useSWR(process.env.SERVER + `/user/profile/${user.email}`, fetcher)
  ...
  const onSubmit = async (values, {setSubmitting, setErrors, setStatus, resetForm}) => {
    try {
      const url = process.env.SERVER + "/user/profile/" + data.email
      const x = await axios.post(url, values);
      if(x.data){
  ...
  const validation = yup.object().shape({
    name: yup.string()
      .max(50, 'Name is 50 chracters max'),
    surname: yup.string()
      .max(50, 'Surname is 50 characters max'),
    phone: yup.string()
      .max(9, 'Phone number has 9 digits'),
    birthday: yup.date()
      .max(new Date(), "You are not that young"),
    pesel: yup.string()
      .length(11, "Pesel has 11 digits")
  });
    ...
    <div className="relative w-screen min-h-screen max-w-full bg-gray-50 flex justify-center justify-items-center flex-col">
        ...
        <div className="min-h-screen py-16 w-full max-w-5xl mx-auto px-4">
          ...
          <div className="w-full px-3 mx-auto">
            ...
            <form id="profileForm" className="space-y-4 py-2 divide-y-2 divide-trueGray-100 max-w-2xl mx-auto" onSubmit={(e) => {
              e.preventDefault()
              formik.handleSubmit()
            }}>
            ...
            <div className="flex flex-col px-4 pt-4">
              <label className="text-xl text-gray-500 font-semibold mb-1 px-2" for="name">Imię {data.name_updated ? <Astrisk />: ""}</label>
              <input id="name" name="name" value={formik.values.name} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className="w-full px-3 py-2 text-sm text-gray-500 font-normal leading-none bg-white outline-none rounded-md" readOnly={data.name_updated}/>
                {formik.touched.name && formik.errors.name ? <ErrorMessage id="errorName" msg={formik.errors.name}/> : null}
            </div>

            <div className="flex flex-col px-4 pt-4">
              <label className="text-xl text-gray-500 font-semibold mb-1 px-2" for="surname">Nazwisko {data.surname_updated ? <Astrisk />: ""}</label>
              <input id="surname" name="surname" value={formik.values.surname} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className="w-full px-3 py-2 text-sm text-gray-500 font-normal leading-none bg-white outline-none rounded-md" readOnly={data.surname_updated}/>
                {formik.touched.surname && formik.errors.surname ? <ErrorMessage id="errorName" msg={formik.errors.surname}/> : null}
            </div>

            <div className="flex flex-col px-4 pt-4">
              <label className="text-xl text-gray-500 font-semibold mb-1 px-2" for="birthday">Data urodzenia {data.birthday_updated ? <Astrisk />: ""}</label>
              <input id="birthday" name="birthday" value={formik.values.birthday} onChange={formik.handleChange} onBlur={formik.handleBlur} type="date"
                className="w-full px-3 py-2 text-sm text-gray-500 font-normal leading-none bg-white outline-none rounded-md" readOnly={data.birthday_updated}/>
                {formik.touched.birthday && formik.errors.birthday ? <ErrorMessage id="errorbirthday" msg={formik.errors.birthday}/> : null}
            </div>

            <div className="flex flex-col px-4 pt-4">
              <label className="text-xl text-gray-500 font-semibold mb-1 px-2" for="pesel">Pesel {data.pesel_updated ? <Astrisk />: ""}</label>
              <input id="pesel" name="pesel" value={formik.values.pesel} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className="w-full px-3 py-2 text-sm text-gray-500 font-normal leading-none bg-white outline-none rounded-md" readOnly={data.pesel_updated}/>
                {formik.touched.pesel && formik.errors.pesel ? <ErrorMessage id="errorpesel" msg={formik.errors.pesel}/> : null}
            </div>

            <div className="flex flex-col px-4 pt-4">
              <label className="text-xl text-gray-500 font-semibold mb-1 px-2" for="phone">Numer telefonu</label>
