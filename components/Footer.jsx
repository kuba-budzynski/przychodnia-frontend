...
function Footer() {
    ...
    const validation = yup.object().shape({
        ...
        email: yup.string()
          .max(100, 'Maksymalnie 100 znaków').email('Podaj poprawny email')
    });
            ...
            <div className="max-w-4xl mx-auto py-5 px-4 sm:px-6 lg:py-8 lg:px-8">
                <div className="flex flex-col items-center lg:flex lg:flex-row justify-around space-x-4">
                    ...
                    <div className="w-3/4 m-0 mt-16 lg:w-1/2 lg:mt-8 xl:mt-0 text-center">
                        ...
                        <p className="mt-4 text-gray-200 text-sm leading-6">Otrzymasz od Nas wiadomości dotyczące nowych usług lub ofert specjalnych</p>
                                ...
                                <div className="rounded-md shadow-sm">
                                    <div>
                                        <input aria-label="Name" name="name" type="text" required 
                                            ...
                                            placeholder="Twoje imię" value={formik.values.name} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                                        {formik.touched.name && formik.errors.name ? <ErrorMessage msg={formik.errors.name} /> : <ErrorMessage msg={""} />}
                                    </div>
                                    <div className="mt-2">
                                        <input aria-label="Email" name="email" type="email" required 
                                            className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" 
                                            placeholder="Adres email" value={formik.values.email} onChange={formik.handleChange} onBlur={formik.handleBlur}/>
                                        {formik.touched.email && formik.errors.email ? <ErrorMessage msg={formik.errors.email} /> : <ErrorMessage msg={""} />}
                                    </div>
                                </div>
